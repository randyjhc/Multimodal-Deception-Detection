...
class HParams(TypedDict):
    ...
    dropout: float
    lr: float
    motion_method: Literal["none", "feature_diff"]
    motion_low: float
    motion_high: float
...
print(
    f"Train: {len(_base_ds)}  |  d_in: {D_IN}  |  motion combos: {len(MOTION_COMBOS)}"
)
...
print(
    f"      motion_method={best['motion_method']}  motion_low={best['motion_low']}  motion_high={best['motion_high']}"
)
...
for epoch in range(final_epochs):
    loss, acc = train_one_epoch(
        final_model, full_train_loader, optimizer, criterion, device
    )
    current_lr = optimizer.param_groups[0]["lr"]
    lr_scheduler.step()
    print(
        f"Epoch {epoch+1}/{final_epochs}  |  LR {current_lr:.2e}  |  Train Loss {loss:.4f}  |  Train Acc {acc:.4f}"
    )

torch.save(
    {"model_state_dict": final_model.state_dict(), "params": best},
    "best_bilstm_final.pt",
)
...
test_loader = DataLoader(
    test_ds, batch_size=BATCH_SIZE, shuffle=False, collate_fn=_collate
)
