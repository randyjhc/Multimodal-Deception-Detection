...
using visual (OpenFace) + audio (OpenSMILE) features.

Usage:
    uv run python run_training_gru.py
"""
...
FUSION_HIDDEN = 128
PATIENCE = 5
SAVE_PATH = "best_bigru.pt"
DEVICE = "cuda"
SEED = 42
# Audio frames are ~10 ms each; subsample to reduce sequence length for the GRU.
# k=50 → ~500 ms resolution, keeping ~400 frames per clip instead of ~19000.
...
def train_one_epoch(
    ...
    for visual_x, visual_len, audio_x, audio_len, y in tqdm(
...
def _make_model(
    visual_d_in: int, audio_d_in: int, device: torch.device
...
def _loader_kwargs(
    ...
    *,
    ...
    kwargs = dict(
        batch_size=batch_size, collate_fn=_multimodal_collate, pin_memory=True
    )
    if seed is not None:
...
if not USE_CV:
    ...
    model = _make_model(visual_d_in, audio_d_in, device)
    ...
    for name, train_vals, val_vals, ylabel in [
        ("loss_curve_gru.png", train_losses, val_losses, "Loss"),
        ("accuracy_curve_gru.png", train_accs, val_accs, "Accuracy"),
    ]:
        plt.figure()
...
# ---------------------------------------------------------------------------
    ...
    base_ds = MultimodalDataset(OPENFACE_ROOT, OPENSMILE_ROOT, "Train", **_ds_kwargs)
    visual_d_in = base_ds.visual_d_in
    audio_d_in = base_ds.audio_d_in
    labels = [lbl for _, _, lbl in base_ds.samples]

    test_ds = MultimodalDataset(OPENFACE_ROOT, OPENSMILE_ROOT, "Test", **_ds_kwargs)
    ...
    for fold, (train_idx, val_idx) in enumerate(splits, start=1):
        ...
        fold_model = _make_model(visual_d_in, audio_d_in, device)
    ...
    for name, vals, ylabel in [
        ("loss_curve_gru.png", final_train_losses, "Loss"),
        ("accuracy_curve_gru.png", final_train_accs, "Accuracy"),
    ]:
        plt.figure()
